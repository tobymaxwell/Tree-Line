...
ggplot(res.all, aes(nature,x=year, color=Species))+geom_line()+facet_grid(Aspect~Site)+
  xlim(1895,2018)
...
library(bfast)
rwi.ts<-rwi.long[is.finite(rwi.long$rwi),]
rwi.ts<-rwi.ts%>%
  group_by(Year)%>%
summarize(rwi=mean(rwi))
rwi.ts<-as.data.frame(rwi.ts)
rwi.ts
rwi.ts<-ts(rwi.ts[2], start=1536)
rwi.ts
breaks<-bfast(rwi.ts, season = "none", max.iter=5,breaks=5)
breaks
plot(log(rwi)~Year, rwi.long)
...
result<-NULL
breakyears<-NULL
for (i in sites[c(-5:-6,-8, -23)]) {
  d<-na.omit(rwi.long[rwi.long$ID==i,])
...
breakyears$Species<-substr(breakyears$ID,3,6)
colnames(breakyears)<-c("B1", "B2", "ID", "Site", "Aspect", "Species")
rwi.minyear<-rwi.long[is.finite(rwi.long$rwi),]%>%
  group_by(ID, Species)%>%
...
rwi.minyear[rwi.minyear$ID=="YNPiAl",]$xmin<-1885
...
rownames(result)<-sites[c(5:6,23)]
rownames(breakyear)<-sites[c(5:6,23)]
...
slopes.2$Species<-substr(slopes.2$ID,3,6)
...
breakyear$Species<-substr(breakyear$ID,3,6)
colnames(breakyear)<-c("B1", "ID", "Site", "Aspect", "Species")
rwi.minyear.2<-rwi.long[is.finite(rwi.long$rwi),]%>%
  group_by(ID, Species)%>%
  summarize(xmin=min(Year))
rwi.minyear.2
...
ggplot(rwi.long[rwi.long$Species=="PiAl",], aes(rwi, x=Year))+
  geom_line()+facet_grid(Site~Aspect)+
  geom_vline(aes(xintercept=B1),color='red', breakyears[breakyears$Species=="PiAl",])+
  geom_vline(aes(xintercept=B2),color='red', breakyears[breakyears$Species=="PiAl",])+
  geom_vline(aes(xintercept=B1),color='red', breakyear[breakyear$Species=="PiAl",])+
  geom_segment(aes(x=xmin, xend=B1, y=Y1, yend=Y2), color="Green",rwi.minyear[rwi.minyear$Species=="PiAl",])+
  geom_segment(aes(x=B1, xend=B2, y=Y3, yend=Y4), color="Orange",rwi.minyear[rwi.minyear$Species=="PiAl",])+
  geom_segment(aes(x=B2, xend=xmax, y=Y5, yend=Y6), color="Purple",rwi.minyear[rwi.minyear$Species=="PiAl",])+
  geom_segment(aes(x=xmin, xend=B1, y=Y1, yend=Y2), color="Green",rwi.minyear.2[rwi.minyear.2$Species=="PiAl",])+
  geom_segment(aes(x=B1, xend=xmax, y=Y3, yend=Y4), color="Orange",rwi.minyear.2[rwi.minyear.2$Species=="PiAl",])+
  ylim(-0.64,6.6)+xlim(1655,2018)+theme_bw()


ggplot(rwi.long[rwi.long$Species=="AbLa",], aes(rwi, x=Year))+
  geom_line()+facet_grid(Site~Aspect, scales='free')+
  geom_vline(aes(xintercept=B1),color='red', breakyears[breakyears$Species=="AbLa",])+
  geom_vline(aes(xintercept=B2),color='red', breakyears[breakyears$Species=="AbLa",])+
  geom_vline(aes(xintercept=B1),color='red', breakyear[breakyear$Species=="AbLa",])+
  geom_segment(aes(x=xmin, xend=B1, y=Y1, yend=Y2), color="Green",rwi.minyear[rwi.minyear$Species=="AbLa",])+
  geom_segment(aes(x=B1, xend=B2, y=Y3, yend=Y4), color="Orange",rwi.minyear[rwi.minyear$Species=="AbLa",])+
  geom_segment(aes(x=B2, xend=xmax, y=Y5, yend=Y6), color="Purple",rwi.minyear[rwi.minyear$Species=="AbLa",])+
  geom_segment(aes(x=xmin, xend=B1, y=Y1, yend=Y2), color="Green",rwi.minyear.2[rwi.minyear.2$Species=="AbLa",])+
  geom_segment(aes(x=B1, xend=xmax, y=Y3, yend=Y4), color="Orange",rwi.minyear.2[rwi.minyear.2$Species=="AbLa",])+theme_bw()

ggplot(rwi.long[rwi.long$Species=="TsMe",], aes(rwi, x=Year))+
  geom_line()+facet_grid(Site~Aspect)+
  geom_vline(aes(xintercept=B1),color='red', breakyears[breakyears$Species=="TsMe",])+
  geom_vline(aes(xintercept=B2),color='red', breakyears[breakyears$Species=="TsMe",])+
  geom_vline(aes(xintercept=B1),color='red', breakyear[breakyear$Species=="TsMe",])+
  geom_segment(aes(x=xmin, xend=B1, y=Y1, yend=Y2), color="Green",rwi.minyear[rwi.minyear$Species=="TsMe",])+
  geom_segment(aes(x=B1, xend=B2, y=Y3, yend=Y4), color="Orange",rwi.minyear[rwi.minyear$Species=="TsMe",])+
  geom_segment(aes(x=B2, xend=xmax, y=Y5, yend=Y6), color="purple",rwi.minyear[rwi.minyear$Species=="TsMe",])+
geom_segment(aes(x=xmin, xend=B1, y=Y1, yend=Y2), color="Green",rwi.minyear.2[rwi.minyear.2$Species=="TsMe",])+
  geom_segment(aes(x=B1, xend=xmax, y=Y3, yend=Y4), color="Orange",rwi.minyear.2[rwi.minyear.2$Species=="TsMe",])+theme_bw()
